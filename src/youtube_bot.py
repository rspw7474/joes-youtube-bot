...
class YouTubeBot(commands.Bot):
    ...
    async def list_subscriptions(self, interaction: discord.Interaction) -> None:
        try:
            ...
            for yt_channel_id in self.data_handler.list_subscriptions(dc_server_id):
                yt_channel_feed = self.feed_checker.youtube_feed_parser.get_yt_channel_feed(yt_channel_id)
    ...
    async def safe_send(self, target_dc_channel: discord.channel.TextChannel, message: str) -> None:
        try:
